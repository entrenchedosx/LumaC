# Physics & Collision Foundation (Phase 28)
...
- Sphere (`radius`), box (`half extents`); capsule DEFERRED.
...
- Scale policy: max `|axis|` multiplies sphere radius and box
  ...
  otherwise; negatives never negate). Sheared or degenerate
  (near-zero axis, NaN) world matrices skip the collider for the
  step — never a silently invalid shape.
...
- Narrow phase: sphere-sphere (+X fallback normal on
  ...
  inside), box-box 15-axis SAT. Single deepest point per pair;
  swapped order reverses the normal.
...
- Serialization: `rigid_body <type> <mass> <lindamp> <angdamp>
  <gscale> <lv*3> <av*3>` (12 tokens) and `collider sphere|box
  ...` (15/17 tokens), `%.9g` floats, strict transactional
...
`collider_pose.c` (world pose + inertia), `broadphase.c`,
`narrowphase.c`, `solver.c`, `step.c` (fixed-step driver),
`events.c`, `query.c`, `scene_physics.c` (capture/apply).
