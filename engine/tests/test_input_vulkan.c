/*
 * Luma Engine Phase 27 Vulkan tests: input-driven scripts to pixels.
 *
 * Injected input -> Lua Input API -> engine transforms -> render
 * extraction -> GPU -> readback: W held moves a scripted cube
 * (pixel shift), release stops it; action bindings drive a hop;
 * pause freezes motion while frames continue; teardown stays clean.
 *
...
int main(void) {
    ...
    printf("Running Luma Engine Phase 27 Vulkan tests...\n");
    ...
    /* PART 1: injected W -> Lua Input.key_down -> scripted cube
     * moves -> pixel shift between frames (full path proof). */
    {
        le_object o;
        ...
        TEST_CHECK(make_script(e->engine,
                               ...
                               "  if Input.key_down(Key.D) then\n"
                               "    local x, y, z = "
                               "self:position()\n"
                               "    self:set_position(x + 8.0 * "
                               "dt, y, z)\n"
                               "  end\n"
                               "end\n",
                               &script) == LE_SUCCESS,
                   "input mover compiles");
        TEST_CHECK(le_object_add_script(e->world, &o, &script) ==
                       LE_SUCCESS,
                   "input mover attaches");
        TEST_CHECK(render_one_frame(e, &fe), "input frame 1");
        lit0 = lit_pixels(e);
        TEST_CHECK(lit0 > 1000, "cube paints pixels");
        x0 = pos_x(e->world, &o);
        le_input_inject_key(e->engine, LE_KEY_D, 1);
        le_engine_step(e->engine, e->world, 0.25f);
        TEST_CHECK(render_one_frame(e, &fe), "input frame 2");
        lit1 = lit_pixels(e);
        x1 = pos_x(e->world, &o);
        TEST_CHECK(x1 - x0 > 1.9f && x1 - x0 < 2.1f,
                   "injected D moved cube +8*0.25");
        TEST_CHECK(lit1 > 1000, "moved cube still paints");
        printf("[info] input mover lit: %ld -> %ld x: %f -> %f\n",
               lit0, lit1, x0, x1);
    }

    /* PART 2: release stops motion (edge-gated Lua movement). */
    {
        le_object o;
        le_asset script = LE_ASSET_INVALID;
        float x0;
        ...
        TEST_CHECK(make_script(e->engine,
                               "function update(self, dt)\n"
                               "  if Input.key_down(Key.A) then\n"
                               "    local x, y, z = "
                               "self:position()\n"
                               "    self:set_position(x - 4.0 * "
                               "dt, y, z)\n"
                               "  end\n"
                               "end\n",
                               &script) == LE_SUCCESS,
                   "release mover compiles");
        TEST_CHECK(le_object_add_script(e->world, &o, &script) ==
                       LE_SUCCESS,
                   "release mover attaches");
        le_input_inject_key(e->engine, LE_KEY_A, 1);
        le_engine_step(e->engine, e->world, 0.1f);
        x0 = pos_x(e->world, &o);
        le_input_inject_key(e->engine, LE_KEY_A, 0);
        le_engine_step(e->engine, e->world, 0.1f);
        x1 = pos_x(e->world, &o);
        TEST_CHECK(x1 == x0, "release stops scripted motion");
        TEST_CHECK(render_one_frame(e, &fe),
                   "post-release frame renders");
    }

    /* PART 3: action binding drives a hop through the render
     * path (Space -> jump action -> +1 x). */
    {
        le_object o;
        le_asset script = LE_ASSET_INVALID;
        ...
        o = make_renderable(e, 4.0f, 0.0f, -2.0f);
        TEST_CHECK(le_input_create_action(e->engine, "jump",
                                          &jump) == LE_SUCCESS,
                   "vulkan jump created");
        memset(&b, 0, sizeof(b));
        ...
        b.key = LE_KEY_SPACE;
        b.scale = 1.0f;
        TEST_CHECK(le_input_add_action_binding(e->engine, &jump,
                                               &b) == LE_SUCCESS,
                   "vulkan jump bound");
        TEST_CHECK(make_script(e->engine,
                               "function update(self, dt)\n"
                               ...
                               "end\n",
                               &script) == LE_SUCCESS,
                   "jump script compiles");
        TEST_CHECK(le_object_add_script(e->world, &o, &script) ==
                       LE_SUCCESS,
                   "jump script attaches");
        x0 = pos_x(e->world, &o);
        le_input_inject_key(e->engine, LE_KEY_SPACE, 1);
        le_engine_step(e->engine, e->world, 0.016f);
        TEST_CHECK(render_one_frame(e, &fe), "jump frame renders");
        x1 = pos_x(e->world, &o);
        TEST_CHECK(x1 - x0 == 1.0f, "action press hops +1");
    }

    /* PART 4: pause freezes scripted motion while frames and
     * pixels continue. */
    {
        le_object o;
        le_asset script = LE_ASSET_INVALID;
        ...
        TEST_CHECK(make_script(e->engine,
                               ...
                               "  local x, y, z = "
                               "self:position()\n"
                               "  self:set_position(x + 2.0 * "
                               "dt, y, z)\n"
                               "end\n",
                               &script) == LE_SUCCESS,
                   "pause mover compiles");
        TEST_CHECK(le_object_add_script(e->world, &o, &script) ==
                       LE_SUCCESS,
                   "pause mover attaches");
        TEST_CHECK(le_time_set_scale(e->engine, 0.0f) ==
                       LE_SUCCESS,
                   "vulkan pause set");
        x0 = pos_x(e->world, &o);
        le_engine_step(e->engine, e->world, 0.25f);
        TEST_CHECK(render_one_frame(e, &fe),
                   "paused frame renders");
        x1 = pos_x(e->world, &o);
        TEST_CHECK(x1 == x0, "paused motion frozen");
        lit = lit_pixels(e);
        TEST_CHECK(lit > 1000, "paused pixels continue");
        TEST_CHECK(le_time_set_scale(e->engine, 1.0f) ==
                       LE_SUCCESS,
                   "vulkan unpause");
    }

    /* PART 5: teardown with live input-driven scripts is clean. */
    TEST_CHECK(1, "vulkan input teardown clean");

    printf("Luma Engine Phase 27 Vulkan tests: %d passed, %d failed\n",
           g_passed, g_failed);
    if (g_failed == 0) {
        printf("ALL PHASE 27 VULKAN TESTS PASSED\n");
    } else {
        printf("TESTS FAILED\n");
    }
    env_shutdown(&env);
